...
write.table(Heterosis_G, file = paste0(cwd,"/Results/Heterosis_G", ".txt"),
  ...
  LocalNucleus = setMisc(x = LocalNucleus, node = "yearOfBirth", value = Gen)
  LocalNucleusRefPop <- c(LocalBulls_Nucleus, LocalCows_Nucleus, LocalNucleus)
  Candidates <- LocalNucleusRefPop[LocalNucleusRefPop@misc>= Gen - 4] 
  LocalBullsNucleus <- selectInd(Candidates[Candidates@misc>= Gen - 1],  sex = "M",
