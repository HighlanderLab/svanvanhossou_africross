...
## Sample farm size
repeat {
  cowSizes <- sample(8:40, size = nFarms, replace = T)
  print(sum(cowSizes))
...
Localcows <- selectInd(LocalBreed20, nInd= sum(cowSizes), trait = "TickCount_local", 
                       use = "pheno", sex = "F")
CowOrder = sample( Localcows@id,size=sum(cowSizes),replace=F) #randomly sample CowIDs 
...
FarmOrder = sample( 1: nFarms,size=nFarms,replace=F)  
