...
for (Gen in 22:40) {
  ...
  for (v in 1:nVillages) {
      for (f in 1:nFarms_v) {
      cat("Working on Farm ", f, " in Village ", v, "\n")
      HybridOffsprings_f[[v]][[f]] <- randCross2(females = HybridCows_f[[v]][[f]], males = HybridBulls_f[[v]][[f]],
      ...
      Candidates2_f[[v]][[f]] <- HybridRefPop_f[[v]][[f]][HybridRefPop_f[[v]][[f]]@misc>= Gen - 1]
      HybridCows_f[[v]][[f]] <- selectInd(Candidates_f[[v]][[f]], nInd(Villages[[v]][[f]]), sex ="F",
                                          trait = selIndex, b= TraitIndex, use ="pheno")
